  ...
  const fetchReservedDates = async () => {
    try {
      const response = await fetch(
        `https://borroapp.azurewebsites.net/api/Reservation/availability/${postId}`,
      );
      if (response.ok) {
  ...
  const handleOpen = async () => {
    ...
    if (!user) {
      try {
        ...
        const response = await fetch(
          `https://borroapp.azurewebsites.net/api/User/${userId}`,
          {
            method: "GET",
            headers: {
  ...
  const handleReserve = async () => {
    ...
    if (dateFrom && dateTo) {
      if (
        ...
        try {
          const response = await fetch(
            `https://borroapp.azurewebsites.net/api/Reservation`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
