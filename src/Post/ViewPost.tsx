...
async function fetchCategory(id: number): Promise<Category> {
  const res = await fetch(
    `https://borroapp.azurewebsites.net/api/Category/${id}`,
    {
      method: "GET",
...
async function FetchPost(id: number) {
  const res = await fetch(`https://borroapp.azurewebsites.net/api/Post/${id}`);
  try {
    if (!res.ok) {
...
async function FetchUserByPostId(id: number) {
  const res = await fetch(
    `https://borroapp.azurewebsites.net/api/UserInfo/postOwner/${id}?postId=${id}`,
    {
      method: "GET",
      headers: { "Content-Type": "application/json" },
    },
