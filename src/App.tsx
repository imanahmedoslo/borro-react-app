...
const AuthProvider = ({ children }: AuthProviderProps) => {
  ...
  useEffect(() => {
    if (localStorage.getItem("token")) {
      ...
      const sessionInfoFromLs: TokenAndId = {
        ...
        id: decodedInfo?.id ?? 0,
        // IsLoggedIn: localStorage.getItem('logInStatus') === 'true' ? true :
        // false,
  ...
  const handleLogin = async (userInfo: CreateUserType) => {
    const response = await fetch(
      `https://borro.azurewebsites.net/api/Login
`,
      {
        method: "POST",
    ...
    <Box
      sx={{
        ...
        "."
         ...
         "."
         "footer"`,
        gridTemplateRows: "auto 0fr 100fr 1fr auto",
        minHeight: "100vh",
      }}
    >
      <LoadScript
